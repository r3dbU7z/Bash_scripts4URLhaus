...
#Set variables
URL_ADDR=$(echo $1 | egrep -o '(f|ht)(tp|tps)://[^ ]+/')
URL_ADDR_HTTP=$(echo $1 | egrep -o '(f|ht)(tp|tps)://[^ ]+/' | sed 's/ftp/http/' )
IP_ADDR=$(echo $URL_ADDR | egrep -o '(f|ht)(tp|tps)://[^ ]+/' | awk -F'//' '{print $2}' | tr -d '/')
...
if [ $# -gt 2 -a "$4" == "-p" ]; then
	...
	URL_ADDR_HTTP=$(echo $1 | egrep -o '(f|ht)(tp|tps)://[^ ]+/' | sed 's/ftp/http/' )${dir_path}/
	echo $URL_ADDR_HTTP
	else 
...
_get_names(){
	...
	eval _names=(`curl -s -k ${URL_ADDR} | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' | \
