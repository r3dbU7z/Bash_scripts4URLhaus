...
URL_ADDR_HTTP=$(echo $1 | egrep -o '[f|ht]+tp://[^ ]+/' )
...
if [ $# -gt 2 -a "$4" == "-p" ]; then
		...
		URL_ADDR_HTTP=$(echo $1 | egrep -o '[f|ht]+tp://[^ ]+/' )${dir_path}/
...
_get_names(){
	...
	eval _names=(`curl -s ${URL_ADDR} | sed '/^d/d' | awk '{print $9}'`)
}
...
if [ $length -gt 0 ]; then #check whether there is load
		...
		mkdir -p ${_dir_path}
		cd ${_dir_path}
		printf  "Get files from FTP-server: $URL_ADDR\n"
